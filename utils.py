...
class DatasetIterater_b(object):
    def __init__(self, batches, batch_size, device):
        self.batch_size = batch_size
        self.batches = batches
        self.n_batches = len(batches) // batch_size
    ...
    def _to_tensor(self, datas):
        ...
        return (x, seq_len, mask), qinggan_y, zhuti_y
...
def build_iterator_bert(dataset, config):
    iter = DatasetIterater_b(dataset, config.batch_size, config.device)
    return iter